...
# 弃用
def _process_features(df_train, df_val, df_test, used_sparse_feats, used_dense_feats, actions, 
                     emb_dim=16, emb_source=None, emb_method='w2v_ns', cat_emb=None, 
                     use_tag_emb=None, use_kw_emb=None, use_tag_text=None,
                     use_kw_text=None, feed_history=None, author_history=None,
                     use_din=False, max_seq_length=128, behavior_feature_list=['feedid','authorid'],
                     ):
    """对特征进行编码，处理出模型所需要的输入
    """
    # 避免更改原输入列表
    ...
    if feed_history in [3, 5]:
        feed_hist = pd.read_pickle(HIST_EMB[feed_history]['feedid'])
        df_train = df_train.merge(feed_hist, on=['userid','date_'], how='left')
        df_val = df_val.merge(feed_hist, on=['userid','date_'], how='left')
        df_test = df_test.merge(feed_hist, on=['userid','date_'], how='left')
        used_dense_feats.extend([i for i in feed_hist.columns if i.startswith('hist_')])
    if author_history in [3,5]:
        author_hist = pd.read_pickle(HIST_EMB[author_history]['authorid'])
        df_train = df_train.merge(author_hist, on=['userid','date_'], how='left')
        df_val = df_val.merge(author_hist, on=['userid','date_'], how='left')
        df_test = df_test.merge(author_hist, on=['userid','date_'], how='left')
        used_dense_feats.extend([i for i in author_hist.columns if i.startswith('hist_')])
    
    # 加上 varlength tag、keyword数据
    if use_tag_text:
        feed_text = pd.read_pickle(f'{DATA_ROOT}/feedid_text_features/feed_tag_kw_padded_text.pkl')[['feedid',use_tag_text]]
        df_train = df_train.merge(feed_text, on='feedid', how='left')
        df_val = df_val.merge(feed_text, on='feedid', how='left')
        df_test = df_test.merge(feed_text, on='feedid', how='left')
        used_varlen_feats.append(VarLenSparseFeat(SparseFeat(use_tag_text, TAG_KW_DIM[use_tag_text][0], 
                                                             embedding_dim=emb_dim), 
                                     TAG_KW_DIM[use_tag_text][1]))
    ...
    for feat in used_sparse_feats:
        lbe = LBE_MODEL[feat]
        df_train[feat] = lbe.transform(df_train[feat].astype(str))+1
        df_val[feat] = lbe.transform(df_val[feat].astype(str))+1
        df_test[feat] = lbe.transform(df_test[feat].astype(str))+1
    ...
    # get DIN features
    if use_din:
        df_din = get_din_history(df_train.append(df_val).append(df_test),
                                 hist_cols=behavior_feature_list, window=5)
        df_train = df_train.merge(df_din, on=['userid','date_'], how='inner')
        df_val = df_val.merge(df_din, on=['userid', 'date_'], how='inner')
        df_test = df_test.merge(df_din, on=['userid', 'date_'], how='inner')
        
        dnn_feature_columns += [VarLenSparseFeat(SparseFeat('hist_'+len(LBE_MODEL[feat].classes_)+1, embedding_dim=emb_dim), 
                                     max_seq_length, length_name="seq_length") for feat in behavior_feature_list]
    
    feature_names = get_feature_names(linear_feature_columns + dnn_feature_columns)
    
    # 3. generate input data for model
    train_x = {name: df_train[name].values for name in feature_names}
    val_x = {name: df_val[name].values for name in feature_names}
    test_x = {name: df_test[name].values for name in feature_names}
    for f in used_varlen_feats:
        train_x[f.name] = np.vstack(df_train[f.name].values).astype(np.int16)
        val_x[f.name] = np.vstack(df_val[f.name].values).astype(np.int16)
        test_x[f.name] = np.vstack(df_test[f.name].values).astype(np.int16)
    
    if use_din:
        train_x['seq_length'] = df_train['seq_length'].values.astype(np.int16)
        test_x['seq_length'] = df_test['seq_length'].values.astype(np.int16)
        val_x['seq_length'] = df_val['seq_length'].values.astype(np.int16)
        for f in dnn_feature_columns:
            if isinstance(f, VarLenSparseFeat):
                train_x[f.name] = pad_sequences(list(train_x[f.name]), maxlen=max_seq_length, padding='post', )
                val_x[f.name] = pad_sequences(list(val_x[f.name]), maxlen=max_seq_length, padding='post', )
                test_x[f.name] = pad_sequences(list(test_x[f.name]), maxlen=max_seq_length, padding='post', )

    return train_x, df_train[actions], val_x, df_val[actions], test_x, linear_feature_columns, dnn_feature_columns, LBE_MODEL
